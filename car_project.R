...
gc<-read.csv("autoscout24-germany-dataset.csv")
str(gc)
#########    Data Processing    #########################
...
#model+make
average_mm<-train_set%>%left_join(average_p_make, by="make")%>%group_by(model)%>%summarise(b_o=mean(price-mu_hat-b_m))
qplot(b_o, data=average_mm)
predicted_mm<-test_set%>%left_join(average_p_make, by="make")%>%left_join(average_mm,by="model")%>%mutate(p=mu_hat+b_m+b_o)%>%pull(p)
predicted_mm%>%filter(is.na(p)==T)%>%select(-iqr,-min,-max,-offerType)
mu_hat+test_set%>%left_join(average_p_make, by="make")%>%left_join(average_mm,by="model")%>%mutate(p=b_m+b_o)%>%filter(is.na(p)==T)
RMSE(predicted_mm,test_set$price)
#horsepower+make+model
average_mmhp<-train_set%>%left_join(average_p_make, by="make")%>%left_join(average_mm, by = "model")%>%group_by(hp)%>%summarise(b_h=mean(price-mu_hat-b_m-b_o))
predicted_mmhp<-test_set%>%left_join(average_p_make, by="make")%>%left_join(average_mm, by= "model")%>%left_join(average_mmhp, by="hp")%>%mutate(p=mu_hat+b_m+b_o+b_h)%>%pull(p)
qplot(b_h, data=average_mmhp)
RMSE(predicted_mmhp,test_set$price)
#car mileage_ratio+horsepower+make+model
average_mmhpmr<- train_set%>%left_join(average_p_make, by="make")%>%left_join(average_mm, by = "model")%>%left_join(average_mmhp, by = "hp")%>%group_by(mileage_ratio)%>%summarise(b_r=mean(price-mu_hat-b_m-b_o-b_h))
qplot(b_r, data=average_mmhpmr)
prediction_mmhpmr<-test_set%>%left_join(average_p_make, by="make")%>%left_join(average_mm, by= "model")%>%left_join(average_mmhp, by="hp")%>%left_join(average_mmhpmr, by="mileage_ratio")%>%mutate(p=mu_hat+b_m+b_o+b_h+b_r)%>%pull(p)
RMSE(prediction_mmhpmr, test_set$price)
#fuel+mileage_ratio+horsepower+make+model
average_mmhpmrf<- train_set%>%left_join(average_p_make, by="make")%>%left_join(average_mm, by = "model")%>%left_join(average_mmhp, by = "hp")%>%left_join(average_mmhpmr, by = "mileage_ratio")%>%group_by(fuel)%>%summarise(b_f=mean(price-mu_hat-b_m-b_o-b_h-b_r))
qplot(b_f, data=average_mmhpmrf)
prediction_mmhpmrf<-test_set%>%left_join(average_p_make, by="make")%>%left_join(average_mm, by= "model")%>%left_join(average_mmhp, by="hp")%>%left_join(average_mmhpmr, by="mileage_ratio")%>%left_join(average_mmhpmrf, by = "fuel")%>%mutate(p=mu_hat+b_m+b_o+b_h+b_r+b_f)%>%pull(p)
